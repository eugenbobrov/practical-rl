...
class XSummaries(gym.Wrapper):
    ...
    def __init__(self, env, prefix=None, running_mean_size=100, step_var=None):
        ...
        os.system(f'mkdir {self.prefix}')
        self.writer = SummaryWriter(self.prefix)
...
def nature_dqn_env(env_id, nenvs=None, seed=None,
    ...
    if nenvs is not None:
        ...
        if summaries:
            env = XSummaries(env, prefix=env_id)
        if clip_reward:
            env = ClipReward(env)
        return env
